...
<head>
    ...
    <link rel="stylesheet" href="manage_game.css">
...
if (isset($_POST['search'])) {
    $search = $_POST['search'];
    $sql = "SELECT * FROM quizzes WHERE name LIKE '%$search%'";
...
<table id="result" class="common-table" style="border-collapse: collapse;">
    <tr class="search-user" style="height: 75px;">
        <th colspan="2" style="font-size:30px; padding-top: 0; padding-bottom: 0; ">Search Results</th>
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        echo '<td><a href="#" onclick="redirectToGameType(\'' . $row['gametype'] . '\',\'' . $row['quizID'] . '\')"><img src="edit_button.png" alt="Edit" width="40" height="30"></a><span class="space">&nbsp&nbsp;</span>';
...
<table id="register" class="common-table" style="border-collapse: collapse;">
    <tr class="manage-user" style="height: 75px;">
        <th colspan="2" style="font-size:30px; padding-top: 0; padding-bottom: 0; ">Manage Games</th>
...
<script>
    function redirectToGameType(gametype, quizID){
        if (gametype === "MCQ"){
            window.location.href = "manage_quiz_mcq.php?quizID=" + quizID;
        }else if (gametype === "rearrange"){
            window.location.href = "manage_quiz_sql.php?quizID=" + quizID;
        }else{
